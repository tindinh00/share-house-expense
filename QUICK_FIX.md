...
## ✅ FIXED: joined_at column error (Dec 16, 2024)

**Error:** `column room_members.joined_at does not exist`

**Solution:** Changed code to use `created_at` instead of `joined_at` since the schema uses `created_at` to track when members joined.

**Files updated:**
- `app/(dashboard)/rooms/[id]/page.tsx` - Changed interface and query from `joined_at` to `created_at`
...
### Bước 4: Refresh trang web (Ctrl+Shift+R)
...
3. **Restart dev server**: 
   ...
   npm run dev
   ```
4. **Check Supabase logs**: Dashboard → Logs → Check for errors

## Giải thích

Lỗi xảy ra vì:
- RLS policies tham chiếu lẫn nhau (rooms ↔ room_members)
- Gây infinite loop khi query

Fix bằng cách:
- Dùng `created_by` thay vì query `room_members`
- Dùng `EXISTS` thay vì `IN` để tránh recursion
- Policies đơn giản hơn, không nested queries

---

**Lưu ý**: Migration này an toàn, không làm mất data. Chỉ fix RLS policies.

**File gốc**: `supabase/migrations/FINAL_FIX.sql`
